        ...
        if let Ok(json) = resp.json::<Value>().await {
            if json["status"] == "success" {
                ...
                return Some(ShodanData {
                    ...
                    asn,
                    ports: vec![80, 443],
                    vulns: vec!["SHODAN API KEY GEREKLİ".to_string()],
                });
            }
        }
    }
        ...
        Ok(resp) => {
            if let Ok(geo_data) = resp.json::<GeoResponse>().await {
                ...
                Json(GeoResponse {
                    ...
                    org: None,
                    as_info: None,
                    message: Some("Parse hatası".to_string()),
                })
            }
        }
...
pub async fn handle_metadata(mut multipart: Multipart) -> impl IntoResponse {
    ...
    while let Ok(Some(field)) = multipart.next_field().await {
        ...
        metadata.insert(
            "Boyut".to_string(),
            format!("{:.2} MB", data.len() as f64 / 1_048_576.0),
        );
...
mod tests {
    ...
    #[test]
    fn test_shodan_data_structure() {
        // Shodan veri yapısının tutarlılığını test eder
        let mock_data = ShodanData {
            city: "Istanbul".to_string(),
            isp: "Turk Telekom".to_string(),
            asn: "AS9121".to_string(),
            ports: vec![80, 443, 22],
            vulns: vec!["CVE-2021-31166".to_string()],
        };
