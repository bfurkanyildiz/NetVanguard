...
const Scanner = () => {
    ...
    const extractMetadata = async (file) => {
        ...
        try {
            ...
            setError("HATA: Sunucuya erişilemiyor!");
        } finally {
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className="hidden"
                                            accept="image/*"
                                            onChange={(e) => extractMetadata(e.target.files[0])}
                                        />
                                        <label
                                            htmlFor="metadata-file"
                                            className={`flex flex-col items-center justify-center gap-3 p-8 border-2 border-dashed rounded-xl cursor-pointer transition-all ${isMetadataLoading ? 'bg-gray-900 border-gray-700 pointer-events-none' : 'bg-[#00ff41]/5 border-[#00ff41]/20 hover:bg-[#00ff41]/10 hover:border-[#00ff41]/40 hover:shadow-[0_0_30px_rgba(0,255,65,0.1)]'}`}
                                        >
                                            {isMetadataLoading ? (
                                                <>
                                                    <span className="text-4xl animate-pulse">📡</span>
                                                    <span className="text-sm font-black text-[#00ff41] animate-pulse">VERİLER AYIKLANIYOR...</span>
                                                </>
                                            ) : (
                                                <>
                                                    <span className="text-4xl opacity-50 group-hover:opacity-100 transition-opacity">📸</span>
                                                    <span className="text-sm font-bold text-[#8abf96]">ANALİZ İÇİN GÖRSEL SEÇ</span>
                                                </>
                                            )}
                                        </label>
                                    </div>
                                    ...
                                    {metadataResults && (
                                        <div className="mt-4 p-4 bg-black/40 border border-[#00ff41]/20 rounded-lg max-h-60 overflow-y-auto text-xs font-mono">
                                            <div className="text-[#00ff41] mb-2 font-black">▶ ANALİZ SONUÇLARI:</div>
                                            {Object.entries(metadataResults).map(([k, v]) => (
                                                <div key={k} className="flex justify-between py-1 border-b border-white/5 last:border-0">
                                                    <span className="text-[#8abf96] font-bold">{k}:</span>
